...
const ProductManagementTable = () => {
  ...
  // Fetch products from API - MODIFIED FOR MASTER DATA (NO STORE FILTERING)
...
//       console.log('Fetching ALL products (master data) with filters:', filters)

      // Use public endpoint to fetch ALL products (no store_uuid filter)
      ...
      // MODIFIED: Fetch all products without store filtering
...
//         console.log('Setting products:', productsData.length, 'Total rows:', total)
        ...
        setTotalRows(total)
        setLastFetchTime(Date.now()) // Track successful fetch time
      } else {
  ...
  }, [debouncedSearch, debouncedStatusFilter, debouncedCategoryFilter, pagination.pageIndex, pagination.pageSize])
...
//       console.log('Detected refresh parameter, forcing data refresh after product edit')
  ...
  }, [searchParams, rbacLoading, fetchProducts, router])
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (hasBeenAway && timeSinceLastFetch > 600000 && !rbacLoading) {
//           console.log('User returned after long absence, refreshing products...')
  ...
  }, [rbacLoading, fetchProducts, lastFetchTime, hasBeenAway])
...
//         console.log('Background refresh: data is very stale and user is active')
  ...
  }, [rbacLoading, fetchProducts, lastFetchTime, isUserActive])
  ...
  const handleExcelExport = () => {
    try {
      ...
      const exportData = products.map((product, index) => ({
        ...
        'SKU': product.sku,
        'Toko': product.toko?.nama_toko || product.toko?.subdomain || '-', // ADDED: Store column
      ...
      const filename = `All_Products_${today}.xlsx`
  ...
  const columns = useMemo<ColumnDef<ProductWithActionsType, any>[]>(() => [
    ...
    // ADDED: Store column
    columnHelper.accessor('toko', {
      header: 'Toko',
      cell: ({ row }) => {
        const storeName = row.original.toko?.nama_toko || '-'
        const subdomain = row.original.toko?.subdomain || ''
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex items-center">
            ...
            <IconButton component={Link} href={`/apps/master-data/produk-management/edit/${row.original.uuid}`} title="Edit Product">
    ...
    <Card>
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 p-5 sm:p-6">
        <div className="flex items-center gap-2">
          <Typography variant="h5">Product Management (All Stores)</Typography>
