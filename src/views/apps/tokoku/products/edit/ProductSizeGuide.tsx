...
const ProductSizeGuide = () => {
  ...
  // Load existing size guide image when component mounts
  useEffect(() => {
    if (formData.size_guide_image && typeof formData.size_guide_image === 'string') {
      const backendUrl = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'
      setSizeGuideImage(`${backendUrl}/storage/${formData.size_guide_image}`)
      setFileName('Gambar panduan ukuran saat ini')
    }
  }, [formData.size_guide_image])
  ...
  const handleRemoveImage = () => {
    ...
    setFileName('')
    setErrorMessage('')
    setFormData({ sizeGuideImage: null, removeSizeGuide: true })
