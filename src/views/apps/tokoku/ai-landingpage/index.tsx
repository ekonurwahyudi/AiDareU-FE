...
const AILandingPageView = () => {
		...
		if (item.data?.html) {
			...
			if (heroSection) {
				...
				if (heroSection.title) {
					const colors = ['#ef0e43', '#fa4f24', '#3b82f6', '#10b981', '#f59e0b', '#8b5cf6']
	...
	const handleGenerate = async () => {
		...
		try {
			...
			// Use coin-based endpoint
			const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ai/generate-landing-page`, {
				method: 'POST',
				headers,
				credentials: 'include',
				body: JSON.stringify({
	...
	useEffect(() => {
		...
		if (token && user) {
			try {
				...
				console.error('AI Landing Page: Error parsing user data:', error)
	...
	return (
		<Container maxWidth={false} className='py-8 px-4'>
				...
				<Box className='relative flex flex-col items-center gap-6 p-10 md:p-14'>
					...
					<Box className='w-full md:is-3/4'>
						...
						<Box className='flex flex-wrap gap-2 justify-center mt-5'>
							{['⚡ Instant Generation (🪙 5 Coin)', '🎨 Professional Design', '📱 Mobile Responsive'].map(feature => (
			...
			<Box className='mt-8'>
				<Card sx={{ borderRadius: 4, border: '1px solid', borderColor: 'divider' }}>
					<CardHeader
						...
						subheader='Daftar landing page yang telah kamu buat dengan AI'
						action={
							<>
						...
						{isLoading ? (
							...
							<Box className='flex items-center justify-center text-textSecondary py-10'>
								<Box sx={{ textAlign: 'center' }}>
									...
									<Typography color='text.secondary'>Mulai buat landing page pertama Anda dengan AI (🪙 5 Coin)</Typography>
								</Box>
							</Box>
						) : (
			...
			<Dialog fullWidth maxWidth='sm' open={openGen} onClose={() => setOpenGen(false)}>
				<DialogTitle>Generate Landing Page dengan AI (🪙 5 Coin)</DialogTitle>
				...
				/>
					...
					<Alert severity='info' icon={<i className='tabler-coins' />}>
						Biaya: 5 Coin untuk landing page lengkap dengan design & copywriting professional
					</Alert>
				</DialogContent>
				<DialogActions>
					...
					>
						{isGenerating ? 'Generating...' : 'Generate (🪙 5 Coin)'}
					</Button>
				</DialogActions>
			</Dialog>
