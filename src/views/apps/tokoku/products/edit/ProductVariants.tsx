...
const ProductVariants = () => {
  ...
  // Load existing variants from formData (for edit mode)
  useEffect(() => {
    if (formData.variants && formData.variants.length > 0 && !isLoaded) {
      const loadedVariants: Variant[] = formData.variants.map((v: any) => ({
        id: v.uuid || v.id,
        name: v.variant_name,
      ...
      const loadedPrices: Record<string, VariantPrice> = {}
      formData.variants.forEach((v: any) => {
        v.options.forEach((opt: any) => {
          const variantKey = `${v.variant_name}-${opt.option_name}`
          loadedPrices[variantKey] = {
            variantName: opt.option_name,
            price: opt.harga || 0,
            stock: opt.stock || 0
          }
        })
      ...
      setVariantPrices(loadedPrices)
      setIsLoaded(true)
    }
  }, [formData.variants, isLoaded])
