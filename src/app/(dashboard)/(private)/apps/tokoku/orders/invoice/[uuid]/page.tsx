...
export default function InvoicePage({ params }: { params: Promise<{ uuid: string }> }) {
  ...
  const fetchInvoiceData = async () => {
    try {
      ...
      if (result.success) {
        ...
        if (data.detail_orders && !data.detailOrders) {
          data.detailOrders = data.detail_orders
        }
        setOrderData(data)
      } else {
  ...
  if (error || !orderData) {
    return (
      <Container maxWidth="md" sx={{ py: 8, textAlign: 'center' }}>
        ...
        <Button onClick={() => router.push('/apps/tokoku/orders')} sx={{ mt: 2 }}>
          Kembali ke Orders
        </Button>
      </Container>
    )
    ...
    <>
      ...
      <Box className="no-print" sx={{ p: 2, display: 'flex', gap: 2, justifyContent: 'center', bgcolor: '#f5f5f5' }}>
        <Button
          ...
          onClick={() => router.push('/apps/tokoku/orders')}
        >
          Kembali
        </Button>
        <Button
      ...
      <Container maxWidth="md" sx={{ py: 4, bgcolor: 'white', minHeight: '100vh' }}>
        ...
        <Box sx={{ display: 'flex', gap: 4, mb: 4 }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            <Typography variant="body2">
              Atas Nama: {bankInfo?.account_holder_name || '-'}
            </Typography>
            <Typography variant="body2">
              No. Rekening: <strong>{bankInfo?.account_number || '-'}</strong>
            </Typography>
        ...
        <Box sx={{ mb: 4 }}>
          ...
          <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 1 }}>
            ...
            {/* Table Body */}
            {orderData.detailOrders?.map((item: any, index: number) => (
              <Box
                key={item.id}
                sx={{
                  display: 'grid',
        ...
        <Box sx={{ textAlign: 'center', mt: 4 }}>
          ...
          <Typography variant="caption" color="text.secondary">
            Invoice ini dibuat secara otomatis oleh sistem AiDareU
          </Typography>
        </Box>
      </Container>
